...
function ValueFormatter(props) {
  try {
    return /*#__PURE__*/React.createElement(React.Fragment, null, props.row[props.column.key]);
  } catch {
    return null;
  }
}
...
function HeaderCell({
  ...
  const className = clsx('rdg-cell', column.headerCellClass, column.resizable && 'rdg-cell-resizable', column.frozen && 'rdg-cell-frozen', column.isLastFrozenColumn && 'rdg-cell-frozen-last', column.alignment === 'right' && 'rdg-cell-align-right');
  const style = {
    width: column.width,
    left: column.left
  };
...
function Cell({
  ...
  className = clsx('rdg-cell', typeof cellClass === 'function' ? cellClass(row) : cellClass, className, column.frozen && 'rdg-cell-frozen', column.isLastFrozenColumn && 'rdg-cell-frozen-last', isCellSelected && 'rdg-cell-selected', isCopied && 'rdg-cell-copied', checkIsDraggedOver() && 'rdg-cell-dragged-over', column.alignment === 'right' && 'rdg-cell-align-right');
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: clsx('rdg-cell-fake-background', (isFilling && draggedOverColumnIdx && draggedOverColumnIdx[0] === column.idx && isDraggedOver && !checkIsDraggedOver() || isCopied) && 'rdg-cell-fake-background-active-left', checkForTopActiveBorder() && 'rdg-cell-fake-background-active-top', checkForBottomActiveBorder() && 'rdg-cell-fake-background-active-bottom', checkForRightActiveBorder() && 'rdg-cell-fake-background-active-right')
  ...
  }), dragHandleProps && /*#__PURE__*/React.createElement("div", {
...
function DataGrid({
  ...
  function handlePaste() {
    ...
    if (!onPaste || !onRowsChange || copiedCells === null || !isCellEditable(selectedPosition)) {
      return;
    }
  ...
  function handleDragEnd() {
    ...
    if (overColIdx !== firstColIdx) {
      ...
      const targetCols = columns.filter((_, i) => i > firstColIdx && i <= overColIdx);
