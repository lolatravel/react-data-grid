  ...
  function handlePaste(text) {
    ...
    for (let i = 0; i < copiedItems.length; i++) {
      for (let ix = 0; ix < copiedItems[i].length; ix++) {
        ...
        if (row && columns[colIdx] && !checkIfCellDisabled(row[columns[colIdx].key]) && updatedTargetRows[startRowIndex + i]) {
          updatedTargetRows[startRowIndex + i] = { ...row,
