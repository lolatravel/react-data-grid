...
function Cell({
  ...
  className = clsx('rdg-cell', typeof cellClass === 'function' ? cellClass(row) : cellClass, className, disabled ? 'rdg-cell-disabled' : isCopied && 'rdg-cell-copied', frozenRightAlign && 'rdg-cell-frozen-align-right' + (scrolledToEnd ? " rdg-cell-frozen-align-right-no-shadow" : ""), column.frozen && 'rdg-cell-frozen', column.isLastFrozenColumn && scrollLeft > 0 && 'rdg-cell-frozen-last', isCellSelected && 'rdg-cell-selected', checkIsDraggedOver() && 'rdg-cell-dragged-over', error && 'rdg-cell-error', alert && 'rdg-cell-alert', column.alignment === 'right' && 'rdg-cell-align-right');
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    ...dragHandleProps
  })), alert && showTooltip && /*#__PURE__*/reactDom.createPortal( /*#__PURE__*/React.createElement("div", {
    ref: setPopper,
    className: "rdg-alert",
    style: styles.popper
  }, alert), document.body));
...
function DataGrid({
  ...
  function commitEditorChanges() {
    ...
    onRowsChange == null ? void 0 : onRowsChange(updatedRows, selectedPosition.row, columns[selectedPosition.idx].key);
  }
  ...
  function handlePaste() {
    ...
    if (!onPaste || !onRowsChange || copiedCells === null || !isCellEditable(selectedPosition) || !cellCanBePasted) {
      return;
    }
    ...
    if (rows.length > 1) {
      ...
      while (checkIndex < filteredRows.length && endRowIndex < rawRows.length) {
        const cell = rawRows[endRowIndex][columnKey];
